...
#' proportions according to the `prop_by` aesthetic defining the denominator
#' (i.e. all proportions for a same value of `prop_by` will sum to 1)
#' 
#' @inheritParams ggplot2::stat_count
#' @param geom Override the default connection between `geom_bar()` and `stat_prop()`.
#' @section Aesthetics:
#' `stat_prop()` requires the \strong{prop_by} aesthetic.
...
#' 
#' p <- ggplot(d) +
#'   aes(x = Class, fill = Survived, weight = Freq, prop_by = Class) +
...
#' 
#' ggplot(d) +
#'   aes(x = Class, fill = Survived, weight = Freq, prop_by = Survived) +
#'   geom_bar(position = "dodge") +
#'   geom_text(stat = "prop", position = position_dodge(0.9), vjust = "bottom")
#'   
#' ggplot(d) +
#'   aes(x = Class, fill = Survived, weight = Freq, prop_by = 1) +
#'   geom_bar() +
#'   geom_text(
...
StatProp <- ggproto("StatProp", Stat,
  ...
  compute_panel = function(self, data, scales, width = NULL) {
   ...
   # sum weights for each combination of prop_by and aesthetics
   ...
   # compute proportions by prop_by
   f <- function(x) {sum(abs(x))}
   panel$prop <- panel$count / ave(panel$count, panel$prop_by, FUN = f)
   panel$width <- width
