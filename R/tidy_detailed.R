...
.levels_and_labels <- function(object, ...) {
  ...
  Levels <- dplyr::mutate_(Levels, variable = ~ rep(
    names(NLevels),
    NLevels
  ...
  Levels <- Levels[, c(
    "term", "variable", "variable_label", "level",
    "level_detail"
  )]
...
.modelFriendlyLevels <- function(f) {
  ...
  if (is.null(lev)) {
    ...
    return(data.frame(level = lev[-1], level_detail = paste0(
      lev[-1],
      " vs. ", lev[1]
...
.form_interaction_labels <- function(s, lnl, numeric_level) {
  ...
  data.frame(
    term = paste0(lnl$term[m], collapse = ":"), variable = paste0(lnl$variable[m],
      collapse = ":"
    ), variable_label = paste0(lnl$variable_label[m], collapse = ":"),
